...
| EXP-07b | Register-decoded B with disjoint 8-column warp ownership (`MT=8/NTT=1`) and two sequential column passes for the full BN128 CTA | Compile-gated `mma-fp16-regb` / `mma-fp32-regb`; source reverted before route execution after mechanism gate failed | Not run by fail-fast rule | Not benchmarked. Resource/SASS gate: K2/K3 REG fp16 125 (>97), fp32 141 (>128), stack/local/spills 0; STS.U16 0; fp16 HMMA/BAR 16/4 vs control 16/3; fp32 24/6 vs control 32/6 | `REJECTED — REVERTED` | `MT=4/NTT=2` is invalid because row-warps would duplicate B decode. The disjoint-column revision removed shared B but could not retain full-tile work within the register/barrier/HMMA ceilings. Evidence: `evidence/EXP-07-mainloop-coopdecode/2026-09-01/EXP-07B-MECHANISM-GATE-REJECTION.md` |
...
| EXP-09 | Full mainloop rewrite mirroring official `escham_code_gemm` warp-local two-band structure: 128×128 CTA as two 128×64 bands; direct warp-local `tile_b` construction from codebook payload (no decoded-B shared store); 2 CTA barriers/tile vs control 3; 9,728 B smem; split-K partial/finalize contract preserved | Compile-gated `ESCHA_MMA_SM120_MAINLOOP_REWRITE_EXPERIMENT`; tags `mma-fp16-mlw`/`mma-fp32-mlw`; source reverted after mechanism-gate failure | Not run by fail-fast rule (pre-timing) | Not benchmarked. Resource/SASS gate FAILED: K2/K3 REG fp16 145/150 (>97), fp32 176 (>128); stack/local/spills 0; BAR 3→2 achieved, decoded-B STS eliminated, HMMA preserved; 4-row-warp decode duplication raised LDS 8→32 (fp16)/16→64 (fp32) and decode ALU ~2–2.7× | `REJECTED — REVERTED` | Shared-B removal costs more registers than it saves at BM128×BN128 (EXP-07 124–154, EXP-09 145–176 vs 97/128 shared-B control); 1 CTA/SM occupancy. Sol mechanism-gate VERDICT=CONFIRM-REJECT; highest-value remaining runtime lever T4 shape-specific ffn_down. Evidence: `evidence/EXP-09-mainloop-rewrite/2026-09-02/` |
| EXP-10 | Cooperative BK32 double-buffered-B temporal pipeline (Decision B, single authorized packed-runtime kernel experiment): decode B[n+1] into second shared slot while MMA(n) consumes B[n]; even/odd warp symmetric phasing; two P/D full-CTA barriers per 16-wide tile vs control 3; cp.async A retained; 17,920 B smem | Compile-gated `ESCHA_MMA_COOP_BK32_EXPERIMENT`; tags `mma-fp16-bk32`/`mma-fp32-bk32`; source reverted after resource-gate failure | Not run by fail-fast rule (pre-timing) | Not benchmarked. Resource/SASS gate FAILED: REG fp16 64 / fp32 82 (below ceilings 97/128 — occupancy fine) but STACK fp16 128 / fp32 256 (control 0) with real LDL/STL spill traffic (~12 main-loop, ~36 epilogue). Sol diagnosis: ptxas homes full accumulator arrays (2×8×2×4=128 B, 2×8×4×4=256 B) due to runtime-indexed acc under unroll-1 helpers | `REJECTED — REVERTED` | Sol FINAL GATE=CONFIRM-REJECT; no bounded fix provable within frozen contract (full unroll cannot prove FP32 ≤128). Packed-runtime incremental kernel program STOPPED per Decision B. Evidence: `evidence/EXP-10-coop-bk32/2026-09-02/` |
